...
class MySqlIntervalTargetModel extends IntervalTargetModel {
  constructor() {
    super();
  }

  static async getAll(req) {
    let query = `SELECT goal_id, title, description, interval_target.id, date_start, date_end, user_id, name, surname, email FROM ${IntervalTargetModel.name} INNER JOIN users ON users.id = user_id INNER JOIN goal ON goal.id = goal_id `;
  ...
  static async getIntervalTarget(id) {
    const query = `SELECT interval_target.id, date_start, date_end, user_id, name, surname, goal_id, title, description FROM ${IntervalTargetModel.name} INNER JOIN users ON users.id = user_id INNER JOIN goal ON goal.id = goal_id WHERE interval_target.id = ?`;
    return await mySqlConnectAndQuery(query, id);
  }

  static async createIntervalTarget(body) {
    const query = `INSERT INTO ${IntervalTargetModel.name} (date_start, date_end, user_id, goal_id) VALUES (?,?,?, ?) `;
  ...
  static async deleteIntervalTarget(id) {
    const query = `DELETE FROM ${IntervalTargetModel.name} WHERE id = ?`;
...
module.exports = MySqlIntervalTargetModel;
